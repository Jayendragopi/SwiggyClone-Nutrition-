...
const FirmCollections = () => {
  ...
  const [excludedFirmNames, setExcludedFirmNames] = useState(["ram tiffins", "siri tiffins","benny fast foods","naidu gari kunda biryani","Siva Restaurant","Priyadarshini","navya tiffins","nww"]); // Add the names you want to exclude here
      ...
      <section className="firmSection">
        {firmData.map((apple) => {
          return apple.firm.map((item) => {
            if (
              (selectedRegion === "All" || item.region.includes(selectedRegion.toLocaleLowerCase())) &&
              !excludedFirmNames.includes(item.firmName) && // Exclude based on firmName
              item.image // Ensure the item has an image
            ) {
              return (
